...
const CheckoutTeamAttendance = ({ navigation }) => {
  const { getAPICall, teamAttendance, setTeamAttendance, getCurretLocation, removeDuplicationRows, userCredential } = useContext(
    DataContext,
  );
  //    For Loader
  ...
  useEffect(() => {
    ...
    setLoading(true);
    var url =
      JsonServer.baseURL + "services/app/Attendance/GetAllUserAttendancewithstartOrEndDate?StartDate=" + moment(new Date()).format("YYYY-MM-DD") + "&EndDate=" + moment(new Date()).format("YYYY-MM-DD") + "&MaxResultCount=10000"
    getAPICall(url, (success, result, error) => {
      
      if (success == true) {
        setLoading(false);
        var getUniqueDataRows = result.items;
        let data = removeDuplicationRows(getUniqueDataRows);
        setState({ data });
        // setFilterdData(data);
        var allusers = [];
  ...
  const handleMarkTeamAttendance = (item) => {
    navigation.navigate("ManagerLocationForStartDuty", { item: item })
  ...
  const renderData = (item) => {
    return (
      <TouchableOpacity
        onPress={() => { handleMarkTeamAttendance(item) }}
        style={{
          padding: 10,
...
export default CheckoutTeamAttendance;
