...
const WOQuestionnaireOptions = ({ navigation, route }) => {
  ...
  useLayoutEffect(() => {
    ...
    AsyncStorage.getItem("AnswerFillingData").then((value) => {
  ...
  const submitQuestionnaires = (counter) => {
    ...
    if (onPageResult) {
      ...
      answerFillingDataState.answers.find(x => x.questionText == QuestionsInitialData[counter].text).answerText = name == null ? selectedOptionIdForDbSend : name;
      ...
      if (QuestionsInitialData[counter].answerType == "Input" && QuestionsInitialData[counter].isQuestionRequired && name == null) {
      ...
      else if (QuestionsInitialData[counter].answerType == "Dropdown" && QuestionsInitialData[counter].isQuestionRequired && QuestionsInitialData[counter]?.options[selectedOptionId]?.option?.name == selectedOptionIdForDbSend && QuestionsInitialData[counter].nestedQuestions && QuestionsInitialData[counter].nestedQuestions[selectedOptionId]?.questions?.length != nestedAnswers.length) {
        ...
        var answersToSend = [];
        answerFillingDataState.answers.forEach(element => {

          var nestedQuestions = nestedAnswers;
          var elementTemp = element;
          delete elementTemp.nestedAnswers;
          answersToSend.push(element);

          nestedQuestions?.length > 0 && nestedQuestions?.forEach(element1 => {
            if (element1.questionText) {
              answersToSend.push(element1);
            } else {
              element1.questionText = element1.text;
              delete element1.text
              answersToSend.push(element1);
            }
        ...
        postRequest(dataToInsert, url, (success, result, error) => {
          if (success) {
            dropDownAlertRef.alertWithType('success', 'Success', "Checklist submitted successfully")
  ...
  const removeDataFromAsyncStorage = async () => {
    await AsyncStorage.removeItem("AnswerFillingData")
    setIsLoading(false)
    setTimeout(() => {
      navigation.pop()
    }, 1200);
  ...
  const getValueFromIncremental = (counter) => {
    ...
    if (onPageResult) {
      ...
      setSelectedOptionIdForDbSend(onPageResult.answerText)
      setName(onPageResult.answerText)
      if (answerFillingDataState.answers[counter].answerType == "Dropdown") {
        answerFillingDataState.answers.find(x => x.questionText == QuestionsInitialData[counter].text).questionText = QuestionsInitialData[counter].text;
        answerFillingDataState.answers.find(x => x.questionText == QuestionsInitialData[counter].text).answerType = QuestionsInitialData[counter].answerType;
        ...
        answerFillingDataState.answers.find(x => x.questionText == QuestionsInitialData[counter].text).selectedOptionIndex = selectedOptionId;
        answerFillingDataState.answers.find(x => x.questionText == QuestionsInitialData[counter].text).imagePath = imgUrl;
        answerFillingDataState.answers.find(x => x.questionText == QuestionsInitialData[counter].text).nestedAnswers = nestedAnswers;
        ...
        answerFillingDataState.answers.find(x => x.questionText == QuestionsInitialData[counter].text).questionText = QuestionsInitialData[counter].text;
        answerFillingDataState.answers.find(x => x.questionText == QuestionsInitialData[counter].text).answerType = QuestionsInitialData[counter].answerType;
        answerFillingDataState.answers.find(x => x.questionText == QuestionsInitialData[counter].text).answerText = name == null ? selectedOptionIdForDbSend == name ? selectedOptionIdForDbSend : name : name;
        answerFillingDataState.answers.find(x => x.questionText == QuestionsInitialData[counter].text).selectedOptionIndex = selectedOptionId;
        answerFillingDataState.answers.find(x => x.questionText == QuestionsInitialData[counter].text).imagePath = imgUrl;
        answerFillingDataState.answers.find(x => x.questionText == QuestionsInitialData[counter].text).nestedAnswers = nestedAnswers;
        setNestedQuestionsData(QuestionsInitialData[selectedId + 1].nestedQuestions?.find(x => x.option.name == onPageResult.answerText))
      ...
      else if (QuestionsInitialData[counter].answerType == "Dropdown" && QuestionsInitialData[counter].isQuestionRequired && QuestionsInitialData[counter]?.options[selectedOptionId]?.option?.name == selectedOptionIdForDbSend && QuestionsInitialData[counter].nestedQuestions && QuestionsInitialData[counter].nestedQuestions.filter(x => x.option.name == QuestionsInitialData[counter]?.options[selectedOptionId]?.option?.name).length > 0
        && QuestionsInitialData[counter].nestedQuestions[QuestionsInitialData[counter].nestedQuestions.filter(x => x.option.name == QuestionsInitialData[counter]?.options[selectedOptionId]?.option?.name).findIndex(x => x.option.name == QuestionsInitialData[counter]?.options[selectedOptionId]?.option?.name)]?.questions?.length != nestedAnswers.length) {
        Alert.alert("Please fill the nested questions")
      }
      else if (QuestionsInitialData[counter].answerType == "Input" && QuestionsInitialData[counter].isImageRequired == true && QuestionsInitialData[counter].imageUrl == null) {
        Alert.alert("Image Required")
  ...
  const getValueFromDecrement = (counter) => {
    ...
    var onPageResult = answerFillingDataState.answers[counter - 1];
    if (onPageResult) {
      onPageResult.nestedAnswers?.length > 0 && setNestedQuestionsData(QuestionsInitialData[selectedId - 1].nestedQuestions?.find(x => x.option.name == onPageResult.answerText))
      setImgUrl(onPageResult.imagePath ? onPageResult.imagePath : '');
      setSelectedOptionId(onPageResult.selectedOptionIndex)
      setSelectedOptionIdForDbSend(onPageResult.answerText)
      setName(onPageResult.answerText)
  ...
  return (
    <SafeAreaView style={{ flex: 1, backgroundColor: constants.colorWhite }}>
      ...
      <View style={{ flex: 1, padding: 20, backgroundColor: '#e6e8f6' }} >
        {isLoading ?
          <View style={{ flex: 1, position: 'absolute', top: "40%", right: 0, left: 0, zIndex: 100, }}>
            ...
            <ScrollView contentContainerStyle={{ flexGrow: 1, justifyContent: "space-between" }}>
              ...
              <View style={{ padding: 10, marginTop: 10 }}>
                ...
                <Text style={{ fontWeight: '700', fontSize: 16, padding: 5 }}>
                  Image
                  {QuestionsInitialData[selectedId]?.answerType == "Dropdown" && QuestionsInitialData[selectedId]?.options[selectedOptionId]?.isImageRequired &&
                    <Text style={{ fontWeight: '700', fontSize: 16, color: constants.colorPrimary }}>
                      {" "}*
                    </Text>}
                </Text>
                ...
                {nestedQuestionsData && optionsViewList.length != 0 ?
                  <TouchableOpacity style={{ flex: 1, backgroundColor: constants.colorPrimary, height: 30, borderRadius: 10, justifyContent: 'center', alignItems: 'center' }} onPress={() => {
                    navigation.navigate("WOQuestionnaireNestedOptions", { templateQuestions: nestedQuestionsData, nestedAnswerArray: nestedAnswerArray, setNestedAnswersFromProps: (nestedQuestions) => fillNestedQuestions(nestedQuestions) })
                  }}>
...
export default WOQuestionnaireOptions
