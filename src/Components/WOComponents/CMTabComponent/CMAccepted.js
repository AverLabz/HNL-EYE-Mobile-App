...
const CMAccepted = () => {
  ...
  const { getAPICall, wOScreenNavigationProps, acceptedWoCM, setAcceptedWoCM } = useContext(DataContext)
    ...
    getAPICall(url, (success, result, error) => {
      if (success == true) {
        if (result.items.length > 0) {
          setAcceptedWoCM(result.items.filter(x => x.workOrderTypeName == "CM"))
  ...
  const handleDetailPress = (item) => {
    wOScreenNavigationProps.navigate("CMAcceptedWODetailPage", { acceptedWosItem: item });
  }
