...
const CMClosed = ({ navigation }) => {

  const { getAPICall, wOScreenNavigationProps, closedWoCM, setClosedWoCM } = useContext(DataContext)
  ...
  useFocusEffect(
    React.useCallback(() => {
      AsyncStorage.getItem("userId").then((value) => {
        getAllClosedWorkOrderCM(JSON.parse(value))
      })
    }, [isFocused])
  )

  const getAllClosedWorkOrderCM = (id) => {

    var url = JsonServer.baseURL + "services/app/WorkOrder/GetAll?MaxResultCount=1000&AssignedEmployeeId=" + id + "&Status=CLOSED";
    getAPICall(url, (success, result, error) => {
      if (success == true) {
        if (result.items.length > 0) {
          setClosedWoCM(result.items.filter(x => x.workOrderTypeName == "CM"))
  ...
  const handleDetailPress = (item) => {
    wOScreenNavigationProps.navigate("WOClosedDetailPage", { pendingWosItem: item });
  }
