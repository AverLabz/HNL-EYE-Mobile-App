...
const PMWOQuestionnaireTemplateName = ({ route, navigation }) => {
  const {
    getAPICall,
    postSubmitWORequest,
    putRequest,
  ...
  const [
    questionsDatoFromAsyncfromAsyncStorage,
    setQuestionsDatoFromAsyncStorage,
  ] = useState([]);
  const [loading, setLoading] = useState(false);

  React.useLayoutEffect(() => {
    navigation.setOptions({
      headerTitle: "PM " + route.params.screenName + " Checklist"
    });

  }, [navigation]);

  useEffect(() => {
    AsyncStorage.getItem('PMWOQuestionsData').then((value) => {
      if (value == null || value == 'null') {
        setLoading(true);
        var url =
          JsonServer.baseURL +
          'services/app/WorkOrder/GetAllQuestionnaireTemplateWithQuestionsByWorkOrderId?Id=' +
          route.params.WOId;
        getAPICall(url, (success, result, error) => {
          ...
          if (success == true) {
            ...
            if (result.items.length > 0) {
              var dataSourceList = [];
              var PMQuestionsTemplates = [];
              PMQuestionsTemplates.push({
                WO: route.params.WOId,
                Data: result.items[0],
              });
              setQuestionsDatoFromAsyncStorage(PMQuestionsTemplates);
              saveToAsyncStoragePMWOQuestionsData(PMQuestionsTemplates);
        ...
        if (
          questionsDatoFromAsync.filter((x) => x.WO == route.params.WOId)
            .length > 0
        ) {
          setQuestionsDatoFromAsyncStorage(questionsDatoFromAsync);
          saveToAsyncStoragePMWOQuestionsData(questionsDatoFromAsync);
          setAllQuestions(
            questionsDatoFromAsync.find((x) => x.WO == route.params.WOId).Data,
          );
          questionsDatoFromAsync
            .find((x) => x.WO == route.params.WOId)
            .Data.questionTemplates.forEach(function (item) {
              dataSourceList.push({
                value: constants.colorMainBg,
                name: item.name,
                questions: item.questions,
          ...
          getAPICall(url, (success, result, error) => {
            if (success == true) {
              ...
              if (result.items.length > 0) {
                var dataSourceList = [];
                var PMQuestionsTemplates = questionsDatoFromAsync;
                PMQuestionsTemplates.push({
                  WO: route.params.WOId,
                  Data: result.items[0],
                });
    ...
    return (
      <TouchableOpacity
        onPress={() => {
          ...
          navigation.navigate('WOQuestionnaireQuestionsMainPage', {
            questionsDatoFromAsyncfromAsyncStorage:
              questionsDatoFromAsyncfromAsyncStorage,
            allQuestions: allQuestions,
            WOId: route.params.WOId,
            WOQuestionnaireQuestionsMainPageItemData: item,
            WOQuestionnaireQuestionsMainPageData: item.questions,
            screenName: item.name,
            siteCode: route.params.siteCode
          });
        }}
        style={{
          height: 70,
    ...
    <View
      ...
      <TouchableOpacity
        ...
        style={[
          ...
          {
            backgroundColor:
              loading === true ? '#aaaaaa' : constants.colorPrimary,
          },
        ]}>
        <Text
          style={[
            ...
            {
              color: loading === true ? '#7d7d7d' : constants.colorWhite,
            },
          ]}>
          Submit
        </Text>
...
export default PMWOQuestionnaireTemplateName;
