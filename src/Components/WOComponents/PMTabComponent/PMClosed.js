...
const PMClosed = () => {
  ...
  const { getAPICall, wOScreenNavigationProps, closedWoPM, setClosedWoPM } = useContext(DataContext)
  ...
  useFocusEffect(
    React.useCallback(() => {
      AsyncStorage.getItem("userId").then((value) => {
        getAllClosedWorkOrderPM(JSON.parse(value))
      })
    }, [isFocused])
  )

  const getAllClosedWorkOrderPM = (id) => {
    ...
    getAPICall(url, (success, result, error) => {
      if (success == true) {
        if (result.items.length > 0) {
          setClosedWoPM(result.items.filter(x => x.workOrderTypeName == "PM"))
