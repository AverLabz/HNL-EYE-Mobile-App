...
import { ActivityIndicator, Alert, FlatList, Image, Keyboard, PermissionsAndroid, SafeAreaView, ScrollView, StatusBar, StyleSheet, Text, TextInput, TouchableOpacity, View } from 'react-native';
...
const AddInventoryQuestionnaire = ({ navigation, route }) => {
    ...
    useLayoutEffect(() => {
        ...
        navigation.setOptions({
            headerTitle: () => (
                <Text style={{ fontSize: 16, fontWeight: "700", color: constants.colorWhite, textTransform: 'capitalize' }}>{route.params.templateQuestions.name}</Text>
            ),
        })
    ...
    const submitQuestionnaires = (counter) => {
        Keyboard.dismiss()
        if (answerFillingDataState.length > 0) {
            if (answerFillingDataState !== undefined) {
                var onPageResult = answerFillingDataState[counter + 1];
                if (onPageResult) {
                    ...
                    answerFillingDataState.find(x => x.text == QuestionsInitialData[counter].text).questionText = QuestionsInitialData[counter].text;
                    answerFillingDataState.find(x => x.text == QuestionsInitialData[counter].text).answerType = QuestionsInitialData[counter].answerType;
                    answerFillingDataState.find(x => x.text == QuestionsInitialData[counter].text).answerText = name == null ? selectedOptionIdForDbSend : name;
                    answerFillingDataState.find(x => x.text == QuestionsInitialData[counter].text).selectedOptionIndex = selectedOptionId;
                    answerFillingDataState.find(x => x.text == QuestionsInitialData[counter].text).imagePath = imgUrl;
                    // route.params.setNestedAnswersFromProps(answerFillingDataState);
                    navigation.goBack()
                } else {
                    ...
                    if (QuestionsInitialData[counter].answerType == "Input" && QuestionsInitialData[counter].isQuestionRequired && name == null) {
                        ...
                        if (answerFillingDataState.findIndex(x => x.text == QuestionsInitialData[counter].text) < 0) {
                            ...
                            answerFillingDataState.push(dataToSendForAnswerFilling);
                        }
                        else {
                        ...
                        var answersToSend = [];
                        answerFillingDataState.forEach(element => {
                            answersToSend.push(element);
                        });

                        answersToSend.forEach(element => {
                            delete element.selectedOptionIndex;
                        });

                        var url = JsonServer.baseURL + 'services/app/SiteAssets_Answer/SaveAnswers'

                        let dataToInsert = {
                            workOrderId: route.params.response.workOrderTypeId,
                            templateId: route.params.response.id,
                            headerName: route.params.templateQuestions.name,
                            remarks: '',
                        ...
                        postRequest(dataToInsert, url, (success, result, error) => {
                            if (success) {
                                setIsLoading(false)
                                dropDownAlertRef.alertWithType('success', 'Success', "Inventory Added successfully")
                                navigation.goBack()
                            } else {
                                setIsLoading(false)
    ...
    const getValueFromIncremental = (counter) => {
        ...
        var onPageResult = answerFillingDataState.length > 0 && answerFillingDataState[counter + 1];
        if (onPageResult) {
                    ...
                    <ScrollView contentContainerStyle={{ flexGrow: 1, justifyContent: "space-between" }}>
                        ...
                        <View style={{ padding: 15 }}>
                            <View style={{ marginBottom: 10 }}>
                                <Text numberOfLines={2} style={[styles.textStyle, { textTransform: 'capitalize' }]}>{QuestionsInitialData[selectedId].text}
...
export default AddInventoryQuestionnaire
