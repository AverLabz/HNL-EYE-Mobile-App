...
const Partial = ({ navigation }) => {

  const { userCredential, getAPICall, wOScreenNavigationProps } = useContext(DataContext)
  let dropDownAlertRef = useRef(null);
  const [userId, setUserId] = useState("")
  const [isLoading, setIsLoading] = useState(false)
  const [text, setText] = useState("")

  const [partialInventory, setPartialInventory] = useState([])
  ...
  const getAllIssuance = (id) => {
    setIsLoading(true)
    var url = JsonServer.baseURL + "services/app/JazzTelcoInventoryConsumption/GetAll?ERPId=" + id + "&Status=PARTIAL"
    getAPICall(url, (success, result, error) => {
      if (success == true) {
        if (result.items.length > 0) {
          setPartialInventory(result.items)
          setIsLoading(false)
        } else {
